...
public class ProductController {
	...
	@GetMapping("create")
	public String create(@ModelAttribute("product") ProductModel product, Model mol) {
		
		List<Categories> cate = this.cateRepo.findAll();
		mol.addAttribute("data", cate);
		mol.addAttribute("view", "/views/admin/products/create.jsp");
		return "layout";
	}

	@PostMapping("store")
	public String store(@Valid @ModelAttribute("product") ProductModel mol, BindingResult result, Model model) {
		if (result.hasErrors()) {
	...
	public String edit(@PathVariable("id") int id, Model mol) {
		...
		mol.addAttribute("view", "/views/admin/products/edit.jsp");
		return "layout";
	}
